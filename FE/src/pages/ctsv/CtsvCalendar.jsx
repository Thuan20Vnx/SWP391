...
const CtsvCalendar = () => {
  ...
  const stats = useMemo(() => {
    ...
    return { total: eventsInMonth.length, pending, school };
  }, [eventsInMonth]);
    ...
    <div className="ctsv-calendar-page">
      <header className="ctsv-events-hero ctsv-cal-hero">
        <div className="ctsv-events-hero-text">
          <span className="ctsv-events-eyebrow">Lịch CTSV</span>
          <h1>Lịch sự kiện toàn trường</h1>
          <p>
            Tổng quan toàn bộ sự kiện toàn trường (cấp trường, đối tác, CLB) — mọi trạng thái, theo tháng.
          </p>
        </div>
          ...
          {loading ? (
            ...
            <ul className="ctsv-cal-sidebar-list">
              {sidebarEvents.map((event) => (
                <li key={event.id}>
                  <Link to={`/ctsv/events/${event.id}`} className="ctsv-cal-sidebar-item">
                    <span
