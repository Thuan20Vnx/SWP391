...
import { fetchCtsvSemesterTimelines } from '../../services/ctsvApi';
import { statusClass } from '../../utils/eventStatus';

const STATUS_FILTERS = [
  { id: '', label: 'Chờ CTSV' },
  { id: 'pending_ctsv', label: 'Chờ duyệt' },
...
const CtsvSemesterTimelineList = () => {
  ...
  const load = useCallback(
    (overrideStatus) => {
      ...
      fetchCtsvSemesterTimelines(params)
  ...
  const pendingCount = useMemo(
    () => timelines.filter((t) => t.statusKey === 'pending_ctsv').length,
    [timelines]
  );
    ...
    <div className="ctsv-events-page">
      <header className="ctsv-events-hero">
        <div className="ctsv-events-hero-text">
          <span className="ctsv-events-eyebrow">CTSV · Kế hoạch kỳ</span>
          <h1>Phê duyệt timeline kỳ CLB</h1>
          <p>
            Timeline đã qua IC-PDP được chuyển sang CTSV để phê duyệt cuối trước khi CLB triển khai kỳ Spring / Summer / Fall.
          </p>
        </div>
            ...
            {loading ? (
              ...
              filtered.map((tl) => (
                <tr key={tl.id}>
                  ...
                  <td>
                    <Link to={`/ctsv/semester-timelines/${tl.id}`} className="ctsv-link-btn">
                      Xem chi tiết
                    </Link>
                  </td>
                </tr>
...
export default CtsvSemesterTimelineList;
