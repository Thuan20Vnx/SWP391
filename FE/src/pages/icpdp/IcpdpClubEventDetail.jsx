    ...
    <div className="ctsv-ed-page">
      ...
      <div className="ctsv-ed-tabs" role="tablist" aria-label="Chi tiết sự kiện">
        {TABS.filter(
          (tab) =>
            tab.id !== 'ma-qr' ||
            ['approved', 'live', 'ended'].includes(event.statusKey || event.status)
        ).map((tab) => (
          <button
            key={tab.id}
            type="button"
      ...
      <div className="ctsv-ed-content">
        ...
        {activeTab === 'ma-qr' && id && ['approved', 'live', 'ended'].includes(event.statusKey || event.status) && (
          <div className="ctsv-ed-panel">
...
export default IcpdpClubEventDetail;
