...
const IcpdpProposalDetail = () => {
  ...
  const handleApprove = async () => {
    ...
    try {
      await icpdpApproveProposal(id, note.trim());
      showToast?.('Đã duyệt nội bộ — đề xuất chuyển sang CTSV duyệt!', 'success');
      setNote('');
      await refresh();
    } catch (e) {
  ...
  const handleRevision = async () => {
    ...
    try {
      await icpdpRequestProposalRevision(id, note.trim());
      showToast?.('Đã gửi yêu cầu chỉnh sửa cho CLB.', 'success');
      setNote('');
      await refresh();
    } catch (e) {
      showToast?.(e.message, 'error');
  ...
  const handleReject = async () => {
    ...
    try {
      await icpdpRejectProposal(id, note.trim());
      showToast?.('Đã từ chối đề xuất CLB.', 'info');
      setNote('');
      await refresh();
    } catch (e) {
      showToast?.(e.message, 'error');
    ...
    <div className="ctsv-ed-page icpdp-detail-page icpdp-proposal-detail-page">
      ...
      {!canIcpdpApprove && proposal.statusKey === 'pending_ctsv' && (
        <div className="icpdp-view-banner">
          ...
          <p>Đề xuất đã được IC-PDP duyệt nội bộ và đang chờ <strong>CTSV duyệt</strong> cấp tiếp theo.</p>
        </div>
      )}
      ...
      {!canIcpdpApprove && proposal.statusKey === 'approved' && (
        <div className="icpdp-view-banner" style={{ borderColor: 'rgba(34, 197, 94, 0.3)', background: '#ffffff' }}>
          ...
          <p style={{ color: '#15803d' }}>Đề xuất đã được CTSV/Admin phê duyệt. Sự kiện đã được tạo chính thức.</p>
        </div>
      )}
        ...
        <div className="ctsv-ed-panel">
          ...
          {(proposal.ctsvNote || proposal.icpdpNote || proposal.rejectionReason) && (
            <div className="icpdp-proposal-notes">
              ...
              {proposal.ctsvNote && (
                <div className="icpdp-proposal-note-card">
                  <span className="icpdp-proposal-note-label">Ghi chú từ CTSV/Admin</span>
        ...
        {canIcpdpApprove && (
          <div className="icpdp-action-panel">
            ...
            <p className="icpdp-action-panel__desc">
              ...
              CTSV/Admin sẽ phê duyệt cấp tiếp theo và tạo sự kiện chính thức.
            </p>
            <label className="icpdp-action-panel__label" htmlFor="icpdp-proposal-note">
              Ghi chú / lý do (bắt buộc khi từ chối hoặc yêu cầu sửa)
            </label>
            <textarea
              id="icpdp-proposal-note"
            ...
            <div className="icpdp-action-panel__buttons">
              ...
              >
                {submittingAction === 'revision' ? (
                  <>
                    <span className="btn-spinner btn-spinner--accent" aria-hidden />
                    Đang xử lý...
                  </>
                ) : (
                  'Yêu cầu chỉnh sửa'
