...
const matchesTimeFilter = (ev, filter) => {
  if (!filter || filter === TIME_FILTERS[0].value) return true;
  ...
  if (filter === TIME_FILTERS[1].value) {
    return eventDay.getTime() === today.getTime();
  }
  if (filter === TIME_FILTERS[2].value) {
    const weekEnd = new Date(today);
    weekEnd.setDate(weekEnd.getDate() + 7);
    return eventDay >= today && eventDay <= weekEnd;
  }
  if (filter === TIME_FILTERS[3].value) {
  ...
  const loadEvents = () => {
    ...
    fetchIcpdpEvents({
      ...
      .catch((err) => {
        ...
        showToast?.('Không tải được sự kiện — kiểm tra backend đang chạy.', 'error');
      })
      .finally(() => {
        setLoading(false);
      });
