...
import ProposalTicketsTable from './ProposalTicketsTable';
...
const AdminClubProposalCard = ({
  ...
  const hasPlan = proposal.hasEventPlan || proposal.eventPlanFile || proposal.eventPlanLink;
  const reviewNotes = getVisibleProposalReviewNotes(proposal);

  return (
    <li className="admin-proposal-card admin-fpt-unit-events__proposal-card">
      ...
      <div className="admin-proposal-card__body">
        <div className="admin-proposal-card__thumb-wrap">
          <img
            src={proposal.image || THUMB_FALLBACK}
            alt=""
      ...
      <footer className="admin-proposal-card__footer">
        <div className="admin-proposal-card__actions-bar">
          <p className="admin-proposal-card__actions-note">
            {proposal.statusKey === 'pending_icpdp'
              ? 'Đề xuất đang chờ IC-PDP duyệt — Admin chỉ xem trước.'
              : 'Phê duyệt hoặc từ chối đề xuất sự kiện CLB.'}
          </p>
...
export default AdminClubProposalCard;
