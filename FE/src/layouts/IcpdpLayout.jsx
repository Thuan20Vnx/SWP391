...
import { expireSessionOnce, logoutWithConfirm } from '../utils/logout';
...
const IcpdpLayout = ({ showToast }) => {
  ...
  const loadUserProfile = useCallback(() => {
    ...
    if (!token) {
      expireSessionOnce(navigate, { showToast });
      return;
    }
    fetch(`${API_BASE}/api/user/profile`, { headers: getAuthHeaders(false) })
      .then((res) => {
        ...
        return res.status === 200 ? res.json() : Promise.reject(new Error('PROFILE_LOAD_FAILED'));
      })
      .then((data) => {
