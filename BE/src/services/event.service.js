...
const EVENT_PUBLIC_LIST_FIELDS =
  'title description thumbnail image category startDate endDate location capacity registeredCount ticketPrice eventState eventType source schoolOrganizerRole status clubId partnerId createdAt postponeReason createdByEmail';

const buildPublicEventQuery = ({ category, state } = {}) => {
  const query = {
  ...
  if (state === 'open') {
    const now = new Date();
    query.eventState = { $nin: ['postponed', 'expired'] };
    query.$or = [
      { endDate: { $gte: now } },
      { endDate: null },
...
const fetchPublicEventsList = async ({ category, state, limit = 300, user } = {}) => {
  const cappedLimit = Math.min(Math.max(Number(limit) || 300, 1), 300);

  const [events, registeredIds] = await Promise.all([
    Event.find(buildPublicEventQuery({ category, state }))
...
const getApprovedEvents = async ({ category, user, limit, state } = {}) =>
...
const getDiscoveryEvents = async ({ category, user, limit = 12, state = 'open' } = {}) =>
  fetchPublicEventsList({
    category,
    user,
    limit,
