  ...
  try {
    if (req.userRole === 'icpdp') {
      const [
        ...
        ClubRegistration.countDocuments({ status: 'pending_icpdp' }),
      ...
      const clubParticipants = clubParticipantAgg[0]?.total || 0;

      return res.json({
        success: true,
        stats: [
          {
            label: 'Đề xuất CLB chờ duyệt',
            value: String(pendingProposals),
            trend: pendingProposals > 0 ? 'Cần xử lý' : 'Không tồn đọng'
          },
          {
            label: 'Đơn thành lập CLB',
            value: String(pendingClubRegistrations),
            trend: pendingClubRegistrations > 0 ? 'Chờ IC-PDP' : 'Đã xử lý'
          },
          {
            label: 'Lượt tham gia sự kiện CLB',
            value: formatCompactCount(clubParticipants),
            trend: activeClubEvents > 0 ? `${activeClubEvents} sự kiện mở` : 'Chưa có sự kiện mở'
          }
        ],
        raw: { pendingProposals, pendingClubRegistrations, activeClubEvents, clubParticipants }
      });
    }
    ...
    return res.json({
      ...
      stats: [
        ...
        { label: 'Sinh viên tham gia', value: formatCompactCount(participants), trend: '+8%' }
      ],
...
router.get('/proposals', async (req, res) => {
  try {
    ...
    if (req.query.status) filter.status = req.query.status;
      ...
      filter.status = { $in: ['pending_ctsv', 'pending_icpdp', 'revision', 'approved', 'rejected'] };
    } else {
      filter.status = { $in: ['pending_ctsv', 'pending_icpdp', 'revision'] };
    }
    if (req.query.q) {
...
router.patch('/proposals/:id/icpdp-request-revision', requireIcpdpOrCtsv, async (req, res) => {
  try {
    if (req.userRole !== 'icpdp') {
      return res.status(403).json({
        success: false,
        message: 'Chỉ IC-PDP được yêu cầu CLB chỉnh sửa ở bước duyệt nội bộ!'
      });
    }
    ...
    if (!note) {
      return res.status(400).json({
        success: false,
        message: 'Vui lòng nhập ghi chú yêu cầu chỉnh sửa!'
      });
    }
